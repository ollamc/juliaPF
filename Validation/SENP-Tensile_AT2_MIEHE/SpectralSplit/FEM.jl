    ...
    function elStiffness(props, jelem, un_el, unp1_el, coords, ndof, statevar, Objective, Residual, Hessian, energySplit)
            ...
            for kintk = 1:ninpt
                    ...
                    if Residual
                            ...
                            # Phase field problem
                            Re[9:12,1] += (Gc*lc*dNdx'*dNdx*unp1_el[9:12,1]+dN[:,1]*((Gc/lc+2.0*PsiPlus)*phi-2.0*PsiPlus)+γ*min(phi-phin,0.0)*dN)*djac
                    end
                    ...
                    if Hessian
                            ...
                            # Phase field problem
                            Ke[9:12,9:12] += (dNdx'*dNdx*Gc*lc+dN*dN'*(Gc/lc+2*PsiPlus)+γ*((phi-phin)<=0.0)*dN*dN')*djac
                    end

                    if Objective
                            Oe += (((1-phi)^2+xk)*PsiPlus+PsiMinus+0.5*Gc*(phi^2/lc + lc*(dNdx*unp1_el[9:12,1])'*(dNdx*unp1_el[9:12,1]))+0.5*γ*min(phi-phin,0.0)^2)*djac
                    end
